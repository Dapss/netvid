        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-slate-800">
                    ...
                    <form method="POST" action="{{ route('serial.update' , $serial->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                            {{-- <div class="grid grid-rows-5"> --}}
                                <div>
                                    <x-label for="judul" :value="__('Judul')" />
                                    <x-input id="judul" class="block my-2 w-full" type="text" name="judul" value="{{$serial->judul}}" autofocus />
                                </div>
                                <div>
                                    <x-label for="tahun" :value="__('Tahun')" />
                                    <x-input id="tahun" class="block my-2 w-full" type="number" min="1888" max="2022" name="tahun" value="{{$serial->tahun}}" autofocus />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    dark:text-gray-300 dark:focus:ring-offset-dark-eval-1" id="sinopsis" cols="30" rows="10" style="resize:none;">{{$serial->sinopsis}}</textarea>
                                </div>
                                <div>
